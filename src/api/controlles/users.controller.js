...
export default {
    ...
    async register(req, res) {
        ...
        if (usernameCount) return badRequest(res, 'Username already used!');
    ...
    async login(req, res) {
        ...
        if (!user || user.password !== password) return badRequest(res, 'Username or password not valid!');
    ...
    async updateBio(req, res) {
        ...
        // Validate body.
        if (!name || !age || !height || !weight) return badRequest(res);

        // Save body.
        // eslint-disable-next-line object-curly-newline
        const postBody = { name, age, height, weight };
        await UserModel.updateOne({ _id: userId }, postBody).exec();
